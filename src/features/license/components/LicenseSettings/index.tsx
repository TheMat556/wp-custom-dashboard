...
export default function LicenseSettings() {
  ...
  if (license.status === "active" && !isGrace) {
    ...
    statusIcon = <KeyOutlined />;
    statusLabel = t("Not activated");
    statusColor = token.colorTextSecondary;
  }

  const featureLabels: Record<string, string> = { chat: "Chat", dashboard: "Dashboard" };

  const expiryLong = formatExpiryLong(license.expiresAt, config.locale ?? "en_US");
