...
export const sessionStore = createStore<SessionState>((set, get) => ({
  ...
  async checkSession() {
    ...
    try {
      // TODO(PR4): This is a platform/session heartbeat against the WordPress core users/me
      // endpoint, not plugin-owned REST traffic. Keep it direct until a dedicated
      // WordPress platform/session client exists.
