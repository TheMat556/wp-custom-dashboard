...
function getBaseUrl(): string {
  return typeof window === "undefined" ? "http://localhost/wp-admin/" : window.location.href;
}

function getAdminPathPrefix(pathname: string): string | null {
...
function isAdminPath(pathname: string): boolean {
  const currentPrefix =
    typeof window === "undefined" ? "/wp-admin" : getAdminPathPrefix(window.location.pathname);

  if (currentPrefix) {
    return pathname === currentPrefix || pathname.startsWith(`${currentPrefix}/`);
  }
...
export function isAdminUrl(url: string): boolean {
  ...
  if (!trimmedUrl || trimmedUrl.startsWith("javascript:")) {
    return false;
  }

  try {
    const parsed = new URL(trimmedUrl, getBaseUrl());
    return parsed.origin === window.location.origin && isAdminPath(parsed.pathname);
  } catch {
    return false;
  }
}
